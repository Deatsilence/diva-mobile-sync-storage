...
test("Returns a error when set() don't have a key", () => {
  expect.assertions(1);
  return SyncStorage.set().catch(error => {
    expect(error).toMatch(
      'set() requires at least a key as its first parameter.',
    );
  });
});
...
test("Returns a error when remove() don't have a key", () => {
  expect.assertions(1);
  return SyncStorage.remove().catch(error => {
    expect(error).toMatch(
      'remove() requires at least a key as its first parameter.',
    );
  });
});
...
test('Can get all keys from storage', () => {
  ...
  expect(all_keys).toEqual(expect.arrayContaining(['foo']));
});
